...
$(document).ready(function () {
  $(document).on('click', 'i[data-type="like"]', function (event) {
    ...
    var like_count = parseInt($(event.target).children('span').text());
    var url = '/news/like';
    var method = 'post';
    var color = '#b80b00';
  ...
  $(document).on('click', 'small[data-type="comment-response"]', function (event) {
    ...
    var news_id = event.target.dataset.news;
    var collapsed = event.target.dataset.collapsed;

    if (collapsed === 'true') {
      event.target.dataset.collapsed = 'false';
      $(event.target).parent().parent().after('' + '<div class="row">' + '<div class="col-10">' + '<input type="text" class="form-control" placeholder="Мысля на мыслю" value="" data-type="response-comment-input">' + '</div>' + '<div class="col-2">' + '<button data-type="comment-response" data-comment="' + comment_id + '" data-news="' + news_id + '" class="btn btn-primary btn block">Отправить</button>' + '</div>' + '</div>');
    }
  });
  $(document).on('click', 'button[data-type="comment-response"]', function (event) {
    var comment_id = event.target.dataset.comment;
    var news_id = event.target.dataset.news;
    var text = $('input[data-type="response-comment-input"]').val().trim();
    console.log(text);
    $.ajax({
      ...
      success: function success(response) {
        window.location('http://localhost:8000/news' + news_id);
      },
      error: function error(jqXHR, textStatus, errorThrown) {
        if (errorThrown === 'Unauthorized') {
          alert('Вы не авторизованны');
...
module.exports = __webpack_require__(/*! C:\project\news\resources\js\script.js */"./resources/js/script.js");
